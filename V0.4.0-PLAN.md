...
| **Phase 0.6** | Multi-Terminal Support (Ghostty) | ✅ COMPLETE |
| **Phase 1** | Workflow Templates | ⏳ Planned |
| **Phase 1b** | Feature Workflow Commands | ⏳ Planned |
| **Phase 1c** | CLI Test Runner | ⏳ Planned |
| **Phase 1d** | Workflow Enforcement | ⏳ Waiting (flow-cli) |
...
### aiterm feature/ghostty-support (In Progress)
- Multi-terminal detection (6 terminals: iTerm2, Ghostty, Kitty, Alacritty, WezTerm, Apple Terminal)
- Ghostty backend with config parsing, theme management
- `ait ghostty` CLI command group (8 commands)
- 19 new tests (all passing)
- Worktree: `~/.git-worktrees/aiterm/feature-ghostty-support`
...
### Phase 0.6: Multi-Terminal Support (Ghostty) ✅ COMPLETE
**Goal:** Extend aiterm beyond iTerm2 to support Ghostty and other modern terminals
**Status:** ✅ COMPLETE (Dec 29, 2025)

**Branch:** `feature/ghostty-support` (worktree: `~/.git-worktrees/aiterm/feature-ghostty-support`)

#### Deliverables

| Component | Description | Status |
|-----------|-------------|--------|
| `terminal/__init__.py` | Terminal type detection (6 terminals) | ✅ |
| `terminal/ghostty.py` | Ghostty backend (250 lines) | ✅ |
| `cli/ghostty.py` | CLI commands (280 lines) | ✅ |
| `tests/test_ghostty.py` | 19 tests (all passing) | ✅ |
...
# Returns: ITERM2, GHOSTTY, KITTY, ALACRITTY, WEZTERM, APPLE_TERMINAL, or UNKNOWN
```
...
ait ghostty config [--edit]     # Show/edit ~/.config/ghostty/config
ait ghostty theme list          # List 14 built-in themes
ait ghostty theme apply <name>  # Apply theme (auto-reloads)
...
| Terminal | Detection | Profile Switch | Theme Switch | Title |
|----------|-----------|----------------|--------------|-------|
| **iTerm2** | ✅ | ✅ | ✅ | ✅ |
| **Ghostty** | ✅ | ❌ (config only) | ✅ | ✅ |
| **Kitty** | ✅ | ❌ | ❌ | ✅ |
...
ait feature release       # Dev → main with validation
```
...
| `ait feature release` | Create PR dev → main |
...
### Phase 1d: Workflow Enforcement (Priority: High)
**Goal:** Enforce `feature/* → dev → main` workflow with smart guards
**Status:** ⏳ Waiting for flow-cli completion
**Dependency:** flow-cli workflow commands (`g feature/promote/release`)

**aiterm CLI Commands:**

| Command | Description |
|---------|-------------|
| `ait workflows enforce --install` | Install pre-push git hooks |
| `ait workflows enforce --uninstall` | Remove git hooks |
| `ait workflows branch-status` | Show pipeline position (rich visualization) |
| `ait workflows branch-rules` | Display branch workflow rules |
| `ait workflows violations` | Show violation history |

**Shared Components:**

| Component | Owner | Description |
|-----------|-------|-------------|
| `g feature/promote/release` | flow-cli | Shell commands (instant) |
| pre-push hook | Shared | Block main/dev, log violations |
| `ait workflows *` | aiterm | Rich CLI management |

**Branch Rules:**

| Branch | Direct Push | Merge From |
|--------|-------------|------------|
| `main` | Blocked | dev, hotfix/* |
| `dev` | Blocked | feature/*, hotfix/*, bugfix/* |
| `feature/*` | Allowed | - |
| `hotfix/*` | Allowed | - |
| `bugfix/*` | Allowed | - |
| `release/*` | Allowed | - |
...
- Violations logged to `~/.claude/workflow-violations.log`
