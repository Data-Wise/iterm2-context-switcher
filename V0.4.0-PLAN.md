...
| **Phase 0.5** | craft Mermaid & Worktree Support | 🔜 NEXT |
...
**Status:** 🔜 IN PROGRESS (Dec 28, 2025)
...
| Task | Effort | Status |
|------|--------|--------|
| Add Mermaid config check to `craft:site:status` | 15m | [ ] |
| Create Mermaid linter skill | 20m | [ ] |
| Update `docs:generate` with Mermaid guidelines | 15m | [ ] |
| Add worktree detection to `craft:check` | 20m | [ ] |

#### Medium Effort (~2.5 hours)

| Task | Effort | Status |
|------|--------|--------|
| Create Mermaid diagram templates command | 1h | [ ] |
| Implement worktree setup command (`craft:git:worktree`) | 1.5h | [ ] |
...
4. Mermaid diagram templates (dependency, workflow, architecture, comparison)
5. Worktree setup automation (stash → checkout → create → pop → install)

**craft v1.8.0 Target:**
- +1 skill (mermaid-linter)
- +2 commands (docs:mermaid, git:worktree)
